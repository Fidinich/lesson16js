...
function wordsList(str, subStr) {
  return new Set(
    str
      ...
      .toLowerCase()
      .split(' ')
      .filter((item) => item.includes(subStr))
  );
}
let myLongStr =
...
function getLocalDate(date, isSecondsn, isISO) {
  let options;
  if (isSecondsn) {
    options = {
      month: 'numeric',
  ...
  if (isISO) {
    let arr2 = date.toLocaleString('ua', options).split('.');
    let arr3 = arr2[2].split(',');
    let arr4 = arr2.concat(arr3);
    arr4.splice(2, 1);
    [arr4[0], arr4[2]] = [arr4[2], arr4[0]];
    arr5 = arr4.splice(3, 1);
    let arr6 = [arr4.join('-')].concat(arr5);
    return arr6.join();
  } else {
...
function getWeekDay(date) {
  ...
  let dateNotStr = new Date(+dateArr[0], +dateArr[1] - 1, +dateArr[2]);
  return dateNotStr.getDay() == 0
    ? 'воскресенье'
    : dateNotStr.getDay() == 1
    ? 'понедельник'
    : dateNotStr.getDay() == 2
...
function getLocalDay(date) {
  let dateArr = date.split('-');
  let dateNotStr = new Date(+dateArr[0], +dateArr[1] - 1, +dateArr[2]);
  return dateNotStr.getDay() == 0 ? 7 : dateNotStr.getDay();
}
...
function getDateAgo(date, days) {
  let dateArr = date.split('-');
  let pastDateNotStr = new Date(
    +dateArr[0],
    +dateArr[1] - 1,
    +dateArr[2] - days
  );
  let month = String(pastDateNotStr.getMonth() + 1);

  month.length == 1 ? (month = '0' + month) : month;

  return `${pastDateNotStr.getDate()}.${month}.${pastDateNotStr.getFullYear()}`;
}
...
let Car = function (engine, model, name, year) {
  ...
  Object.defineProperty(this, 'used', {
    get() {
      return this.year == now ? 'new' : 'used';
    },
    set(value) {
      if (value == 'new' && this.year !== now) return (this.year = now);
    },
  });
